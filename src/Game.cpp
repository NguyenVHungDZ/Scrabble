...
Game::Game()
    : isRunning(false), currentState(PLAYING), highScore(0), window(nullptr), renderer(nullptr),
      ...
      selectedTile(nullptr), mouseX(0), mouseY(0), isFirstTurn(true) {}
...
bool Game::init()
{
    ...
    {
        cerr << "ERROR: Failed to load manual icon!" << endl;
        return false;
    }
...
void Game::submitWord()
{
    ...
    if (isFirstTurn)
    {
        ...
        { // Thêm getter getTempPlacedTiles() trong Board
    ...
    if (allWordsValid)
    {
        ...
        for (const auto &placement : placements)
        {
            for (Tile *tile : placement.tiles)
            {
                ...
                { // You'll need to add a simple getter for this
                    if (tile == tempTile)
                    {
                        isNewTile = true;
                        break;
                ...
                if (isNewTile)
                {
                    ...
                    if (bonus == DOUBLE_WORD || bonus == CENTER)
                        wordMultipliers *= 2;
...
void Game::renderUI()
{
    ...
    string scoreText = "Current Score: " + to_string(player->getScore());
    string livesText = "Resets Left: " + to_string(player->getLives());
...
void Game::renderManual()
{
    ...
    vector<string> manualLines = {
        ...
        "6. Speacial tiles on the board give bonuses:",
        "   - DL: Double Letter Score",
        "   - TL: Triple Letter Score",
        "   - DW: Double Word Score",
        "   - TW: Triple Word Score",
        "7. One of the first word's letter must be placed on the center tile.",
        "",
...
void Game::cleanup()
{
    ...
    if (manualIconTexture)
        SDL_DestroyTexture(manualIconTexture);
    if (renderer)
